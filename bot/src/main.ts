...
bot.command("like", async (ctx) => {
    ...
    try {
        ...
        // Download the photo
        const photoBuffer = await ctx.api.getFile(photoFile.file_id);
        const photoUrl = `https://api.telegram.org/file/bot${ctx.environment.botToken}/${photoBuffer.file_path}`;
        ...
        if (!width || !height) {
            await ctx.reply("Could not process image dimensions");
            return;
        }

        // Calculate watermark dimensions (1/3 of original image)
        ...
        const processedImageBuffer = await sharp(Buffer.from(originalImageBuffer))
            .composite([{
                ...
                gravity: 'southeast' // This positions the overlay in the bottom right
            }])
            .toBuffer();

        // Send the processed image
        await ctx.replyWithPhoto(
            new InputFile(processedImageBuffer, 'output.jpg')
            ,
            {
